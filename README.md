...
> 从零开始，手把手教你安装和使用Boss Agent。跟着做就行！
...
> ✅ 导入成功后，你会看到扩展列表里出现了「Boss Agent」插件，右上角扩展图标栏也会出现Boss Agent的图标。

---

### 第四步：获取 DeepSeek API Key（免费）

Boss Agent的 AI 分析功能需要 DeepSeek 的 API Key，**新用户有免费额度**，够用很久。
...
### 第五步：配置Boss Agent（首次使用向导）

1. 点击 Chrome 右上角的 **拼图图标** 🧩（扩展管理按钮）
2. 找到 **「Boss Agent」**，点击图标打开新手引导页面
...
3. 你会看到页面上出现Boss Agent的 **悬浮按钮**（💡 灯泡图标）
4. 点击它，选择 **「深度剖析」**
...
2. 点击Boss Agent悬浮按钮，选择 **「开启巡航模式」**
3. Boss Agent会自动逐个扫描当前页面的所有职位
4. 扫描完成后：
   - 🟢 绿色边框的 = 高匹配，值得投
   - ⚪ 灰色半透明的 = 低匹配，可以跳过
5. 点击感兴趣的职位即可查看详细分析
...
1. 在 Boss直聘的**职位列表页**
2. 点击Boss Agent悬浮按钮，选择 **「🔁 自动沟通循环」**
3. Boss Agent会自动开始工作：
   - 自动打开每个职位的详情页
   - AI 根据你的简历 + 岗位 JD，生成**个性化的打招呼话术**
   - 自动填入并发送打招呼消息
   - 自动返回列表，继续处理下一个职位
4. 整个过程**完全自动**，你可以去喝杯咖啡 ☕
5. 完成后可以在页面控制台中**导出沟通日志 CSV**，复盘投递情况

> 💡 打招呼话术是 AI 根据你的简历和每个岗位的 JD 定制的，不是千篇一律的模板！

#### 方式四：AI 生成打招呼用语

1. 在 Boss直聘上点击某个职位的 **「立即沟通」** 按钮，进入聊天窗口
2. Boss Agent会检测到聊天界面，自动出现 **「AI 生成打招呼」** 按钮
3. 点击它，AI 会根据你的简历和当前岗位 JD 生成个性化打招呼话术
4. 话术会**自动填入**输入框，确认后直接发送即可
