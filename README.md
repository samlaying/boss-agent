...
### 🤖 自动投递（自动沟通循环）
- **一键启动**：在职位列表页开启自动沟通循环，自动逐个投递
- **AI 个性化打招呼**：根据你的简历 + 岗位 JD，AI 自动生成针对性的打招呼话术
- **自动发送**：生成话术后自动填入并发送，全程无需手动操作
- **智能兜底**：AI 生成失败时自动使用默认话术，不会卡住
- **日志导出**：自动沟通日志支持导出 CSV，方便复盘投递记录
...
### 💬 AI 智能打招呼
- **简历匹配生成**：提取你简历的核心亮点，结合 JD 生成个性化打招呼话术
- **一键填入**：在 Boss直聘聊天窗口中自动填入 AI 生成的话术
- **自定义 LLM**：支持配置自定义的 AI 模型来生成打招呼用语
