...
必须针对该职位生成 **{{SCRIPT_COUNT}} 种不同风格** 的开场白 (opening_scripts){{SCRIPT_STYLES}}
**必须严格输出 {{SCRIPT_COUNT}} 个对象。**
**字数限制**：每条开场白严格控制在 80 字以内；不要出现“你好HR/Hello HR/Dear Manager”等客套开头，直接输出正文。
...
- **Hooks (亮点/机会)**: 提炼 1-2 个最吸引人的卖点。Label 必须包含图标：
  ...
  - 🚀 = 能力溢出 (如: 降维打击/带队经验)
  - 例如: "✅ 经验对口", "🚀 能力溢出"
- **Redemption (挽尊)**: 如果分数 < 60 但核心能力匹配，必须生成此字段。告诉用户："虽然分数低，但因为 [核心强项]，值得一试"。
...
// === 处理直连调用 ===
async function handleDirectCall(apiKey, systemPrompt, resume, jobText, hrName, bossTitle, sendResponse, tabId, greetingCount) {
    // 准备 System Prompt
    let sysPrompt = prepareSystemPrompt(systemPrompt, greetingCount);
...
// === 处理 Serverless 调用 ===
async function handleServerlessCall(clientId, systemPrompt, resume, jobText, hrName, bossTitle, sendResponse, tabId, userKey, greetingCount) {
    // 准备 System Prompt
    let sysPrompt = prepareSystemPrompt(systemPrompt, greetingCount);
...
// === 辅助函数：准备 Prompt ===
const SCRIPT_STYLES_MAP = {
    1: '\n1. 🛡️ 稳健防守型：适合 HR，礼貌、专业、强调匹配度。',
    2: '\n1. 🛡️ 稳健防守型：适合 HR，礼貌、专业、强调匹配度。\n2. ⚔️ 痛点狙击型：适合业务主管，直接抛出解决方案，展示野心。',
    3: '\n1. 🛡️ 稳健防守型：适合 HR，礼貌、专业、强调匹配度。\n2. ⚔️ 痛点狙击型：适合业务主管，直接抛出解决方案，展示野心。\n3. 🔥 真诚破冰型：适合跨行或错配，用真诚和学习力打动对方。',
    5: '\n1. 🛡️ 稳健防守型：适合 HR，礼貌、专业、强调匹配度。\n2. ⚔️ 痛点狙击型：适合业务主管，直接抛出解决方案，展示野心。\n3. 🔥 真诚破冰型：适合跨行或错配，用真诚和学习力打动对方。\n4. 🤝 价值共鸣型：找到与面试官的共同价值观，建立情感连接。\n5. 🚀 野心展示型：直接表达对职位的强烈兴趣和快速产出的决心。',
};

function prepareSystemPrompt(userSystemPrompt, greetingCount = 3) {
    ...
    // 替换占位符为实际 greetingCount
    const count = Math.max(1, Math.min(5, greetingCount || 3));
    const styles = SCRIPT_STYLES_MAP[count] || SCRIPT_STYLES_MAP[3];
    sysPrompt = sysPrompt.replace(/\{\{SCRIPT_COUNT\}\}/g, String(count));
    sysPrompt = sysPrompt.replace(/\{\{SCRIPT_STYLES\}\}/g, styles);
