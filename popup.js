...
实习经历
（请填写实习经历，如：时间段  公司  岗位，及主要工作内容）

项目经历
（请填写项目经历，如：时间段  项目名称  角色，及主要成果）

技能与技术
（请填写掌握的技能，如：AI、PM、SQL 等）`;
...
document.addEventListener('DOMContentLoaded', () => {
  ...
  document.getElementById('copyPromptBtn').addEventListener('click', () => {
    const promptText =
      '请将提供的文档内容100% 完全复刻提取,转化为markdown 格式，输出结果需可直接复制粘贴使用，无需额外内容';
    navigator.clipboard
  ...
  const confirmKeyBtn =
    document.getElementById('confirmKeyBtn') || document.getElementById('redeemKeyBtn');
  const secretKeyInput =
    document.getElementById('secretKeyInput') || document.getElementById('redeemKeyInput');
...
function loadSettings() {
  chrome.storage.local.get(
    ...
    (res) => {
      ...
      if (res.filterActiveHr !== undefined)
        document.getElementById('filterActiveHr').checked = res.filterActiveHr;
      if (res.enableLabMode !== undefined)
        document.getElementById('enableLabMode').checked = res.enableLabMode; // 新增
      ...
      if (res.chatSystemPrompt)
        document.getElementById('chatSystemPrompt').value = res.chatSystemPrompt;
      ...
      document.getElementById('scanPauseSeconds').value =
        res.scanPauseSeconds !== undefined ? res.scanPauseSeconds : 5;
      document.getElementById('autoPauseThreshold').value =
        res.autoPauseThreshold !== undefined ? res.autoPauseThreshold : 80;
      document.getElementById('historyRetentionDays').value =
        res.historyRetentionDays !== undefined ? res.historyRetentionDays : 15;
      document.getElementById('maxHistoryRecords').value =
        res.maxHistoryRecords !== undefined ? res.maxHistoryRecords : 1000;
    }
  );
}
...
function setModalStatus(msg, isError) {
  ...
  document.getElementById('modalButtons').style.display =
    isError || msg.includes('已生成') ? 'flex' : 'none';
...
function applyAIConfig(closeAfter = true) {
  ...
  document.getElementById('keywordArsenal').value = JSON.stringify(
    tempGeneratedConfig.keyword_arsenal,
    null,
    2
  );
